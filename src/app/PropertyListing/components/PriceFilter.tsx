...
const PriceFilter: React.FC<PriceFilterProps> = ({ onFilterChange }) => {
	...
	const handleBlur = (index: number) => () => {
		...
		} else if (newValue[index] > 10000000) {
			newValue[index] = 10000000;
		}
		setValue(newValue);
	};
				...
				<div className='absolute -left-44  md:left-0 top-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-10 w-[1000px] max-w-sm'>
					<Box sx={{ padding: '16px' }}>
						...
						<p className='text-sm font-medium mb-4'>
							From ${formatNumber(value[0])} to ${formatNumber(value[1])}
						</p>
								...
								<div className='flex items-center border-2 rounded-lg border-gray-800 relative'>
									<Input
										...
										inputProps={{
											...
											min: 10000,
											max: 10000000,
											type: 'text', // Use text to support formatting
